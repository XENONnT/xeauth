...
class XeTokenRequest(XeAuthStep):
    ...
    def prompt(self, p):
        print(
            f"Please visit the following URL to complete "
            f"the login: {self.verification_uri_complete}",
            file=p.console,
        )
        if p.open_browser:
            import webbrowser
    ...
    def fetch_token(
        self, oauth_domain, oauth_token_path, device_code, client_id, headers={}
    ):
